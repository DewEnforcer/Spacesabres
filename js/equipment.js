  ...
  function drop(ev) {
    ...
    if (itemData[0] == slotData[0]) {
      ...
      $(".eq_result").html("<h2>Error!</h2><p>You cannot equip this type of item into this slot!</p><button type='button' class='btn_eq_confirm'>Ok</button>");
    }
  }
  ...
  function eqAjax(action, newEq, newInv, shipType) {
  ...
  $(document).ready(function() {
    $(document).on("contextmenu", ".item_img", function() {
    ...
    $(".btn_save_setup").click(function () {
      if ($(this).css("cursor") == "pointer" || action == "template") {
        ...
        eqAjax(action, JSON.stringify(equipmentArr),JSON.stringify(invArr), shipType).then(data => {
          $(".eq_loader").remove();
