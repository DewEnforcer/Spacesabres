$(document).ready(function(){
  $(document).on("click", ".planet", function(planet){
    let mapID = $(".map_input").val();
    let elementInfo = planet.target.id;
    ...
    $(".galaxy_main_map_planet_params").css("display", "flex");
    $(".galaxy_main_map_planet_params").html("<img src='../image/graphics/closeMsg.png' class='close_planet_info' style='max-width:30px; cursor:pointer; margin-left: calc(100% - 30px);'><span class='targetNick' id='"+decodedParams["owner"]+"'>Owner of the battlestation near this planet: "+decodedParams["owner"]+"</span><span>Coordinates X: "+decodedParams["coordsx"]+"</span><span>Coordinates Y: "+decodedParams["coordsy"]+"</span><span>Map: "+decodedParams["mapLocation"]+"</span>");
    if (decodedParams["attack"] == 0 && type != "Passive" && type != "Ally") {
      $(".galaxy_main_map_planet_params").append("<a href='include/redirect.php?action=briefing&&coordsX="+decodedParams["coordsx"]+"&&coordsY="+decodedParams["coordsy"]+"&&map="+decodedParams["mapLocation"]+"'>Attack this station!</a>");
  ...
  $(".backwards").click(function(mapbackwards){
    ...
    $(".navigation_info").html("Current system displayed in the navigation: "+newmap+"");
    $(".map_input").val(newmap);
    $(".galaxy_main_map").html('<img src="../image/graphics/sungalaxy.png" alt="sun" style="position:absolute; left:372.5px;top:177.5px;">');
    $(".galaxy_main_map").append(parsemap);
  }
  });//end ajax call
  })
...
$(document).on("click", ".btn_galaxy_hack", function(attemptHack) {
  ...
  $(".galaxy_main_map_hack_attempt").fadeIn(1000, function() {
      ...
      $(".galaxy_main_map_hack_attempt").html("<p>Attempting to hack subspace network communications of: "+targetNick+"</p><p class='time_hack_countdown'></p>");
