...
function generateFleets($maxPoints, $coef, $rank) {
  $shipParams = [[800,1040,1280,3200,4800,8000], [0,0,5000,30000,35000,75000], [10000,12500,20000,60000,72500,225000], [0.1,0.1,0.15,0.05,0.05,0]];
  $arrayFleetNumbers = [];
  $fleet = [];
  $arrayStats = [];
  ...
  for ($i=0; $i < 6; $i++) {
    for ($k=0; $k < $arrayFleetNumbers[$i]; $k++) {
      $shipObj = ["type"=>$i, "stats"=>[$shipParams[0][$i], $shipParams[1][$i], $shipParams[2][$i], $shipParams[3][$i]]];
