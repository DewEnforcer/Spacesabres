...
if ($victory == "attacker" || $victory == "defender" || $victory == "drawDestruction") {
  $end = true;
} else {
  $end = false;
}
...
if ($SelectedAttacks["rounds"] == 3 || $end == true) {
  $rewards = addRewards($conn, $targetUserID, $userID);
  $defenderRewards = "<p align='center'>Your rewards: ".number_format($rewards[1][0], '0', '.',' ')." Credits, ".number_format($rewards[1][1], '0', '.',' ')." Hyperids, ".number_format($rewards[1][2], '0', '.',' ')." Natiums</p>";
  $attackerRewards = "<p align='center'>Your rewards: ".number_format($rewards[0][0], '0', '.',' ')." Credits, ".number_format($rewards[0][1], '0', '.',' ')." Hyperids, ".number_format($rewards[0][2], '0', '.',' ')." Natiums</p>";
  ...
  if ($targetUserID > 1) {
  checkMsgs($conn, $to1);
  sendMsg($conn, $to1, $from, $currentTime, $subject, $fullMsg, $defenderRewards , $token);
  }
  sendMsg($conn, $to2, $from, $currentTime, $subject, $fullMsg, $attackerRewards , $token);
} else {
  "";
}
 ?>
