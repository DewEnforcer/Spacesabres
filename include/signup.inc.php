...
if(isset($_POST["signup-submit"])){
  ...
  $tosAgreement = $_POST["rule_agreement"];
  $newsLetter = $_POST["newsletter_agreement"];
  ...
  elseif ($pwd !== $pwdRepeat){
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCheck > 0) {
        ...
        $sql = "INSERT INTO notactiveusers (Username, Email, Password, userID, regDate, idActivate, idExpire, coordsX, coordsY, mapLocation, IPreal, IPproxy, newsletter) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          header("Location: ../index.php?error=".mysqli_error($conn)."");
          exit();
        ...
        mysqli_stmt_bind_param($stmt, "sssiisiiiissi", $username, $email, $hashedPwd, $generatedID, $date, $id, $expireDate, $coordsPage[0], $coordsPage[1], $maximalMap, $realIP, $proxyIP, $newsQuery);
