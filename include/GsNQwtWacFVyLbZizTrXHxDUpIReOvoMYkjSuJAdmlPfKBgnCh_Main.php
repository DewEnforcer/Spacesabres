...
// points functions
function countRewards($conn, $player, $hornet, $spacefire, $starhawk, $peacemaker, $centurion, $nathalis, &$rewardsHyperids, &$rewardsNatiums , &$rewardsCredits , $userID) {
  ...
  $sql = mysqli_query($conn, "SELECT $player FROM usermovement WHERE userID=$userID");
  $rewards = mysqli_fetch_assoc($sql);

  $rewards = unserialize($rewards[$player]);

  $rewards[0] += $creditsReward;
  $rewards[1] += $hyperidsReward;
  $rewards[2] += $natiumsReward;

  $rewards = serialize($rewards);
  $sql = mysqli_query($conn, "UPDATE usermovement SET $player='$rewards' WHERE userID=$userID");
  return true;
}
function addRewards($conn, $targetUserID, $userID) {
  $sql = mysqli_query($conn, "SELECT rewardsAttacker, rewardsDefender FROM usermovement WHERE userID=$userID");
  $rewards = mysqli_fetch_assoc($sql);

  $rewardsA = unserialize($rewards["rewardsAttacker"]);

  $rewardsD = unserialize($rewards["rewardsDefender"]);

  $sql = mysqli_query($conn, "SELECT credits, hyperid, natium FROM users WHERE userID=$userID");
  $attackerValutes = mysqli_fetch_assoc($sql);

  $sql = mysqli_query($conn, "SELECT credits, hyperid, natium FROM users WHERE userID=$targetUserID");
  $defenderValutes = mysqli_fetch_assoc($sql);

  $newCredsAttacker = $attackerValutes["credits"] + $rewardsA[0];
  $newHyperidsAttacker = $rewardsA[1]+$attackerValutes["hyperid"];
  $newNatsAttacker = $rewardsA[2]+$attackerValutes["natium"];

  $newCredsDefender = $defenderValutes["credits"] + $rewardsD[0];
  $newHyperidsDefender = $rewardsD[1]+$defenderValutes["hyperid"];
  $newNatsDefender = $rewardsD[2]+$defenderValutes["natium"];

  $sql = mysqli_query($conn, "UPDATE users SET credits=$newCredsAttacker,  hyperid=$newHyperidsAttacker, natium=$newNatsAttacker WHERE userID=$userID");
  $sql = mysqli_query($conn, "UPDATE users SET credits=$newCredsDefender,  hyperid=$newHyperidsDefender, natium=$newNatsDefender WHERE userID=$targetUserID");

  $rewards = [$newCredsAttacker, $newHyperidsAttacker, $newNatsAttacker];
  $rewardsD = [$newCredsDefender, $newHyperidsDefender, $newNatsDefender];
  manageMissions($userID, $rewards, $conn);
  manageMissions($targetUserID, $rewards, $conn);
  return [[$rewardsA[0],$rewardsA[1],$rewardsA[2]], [$rewardsD[0], $rewardsD[1], $rewardsD[2]]];
}
...
function sendMsg($conn, $to, $from, $currentTime, $subject, $message, $rewards, $token) {
  $message .= $rewards;
...
$sql = mysqli_query($conn, "DELETE FROM usermovement WHERE userID=$userID AND id=$attackID");
  ...
  if ($missionType == 2 || $missionType == 3) {
    ...
    $status = "Daily bonus from the Spacesabres.";
  } elseif ($missionType == 5) {
    $status = "Items recieved from the payment.";
  } else {
    $status = "";
  }
...
// goes through all attacks that arrived at the enemy base
 ...
 if (mysqli_num_rows($selectAttacks) > 0) {
   while($SelectedAttacks = mysqli_fetch_assoc($selectAttacks)) {
     ...
     $timeSet = $SelectedAttacks["setAttack"];
     $timeAttack = $SelectedAttacks["travelTime"];
