<?php
  ...
  $Fleet = mysqli_fetch_assoc($getFleet);
    ...
    if($newCreds>=0 && $newHyperid>=0 && $newNatium>=0){
      if ($shipID < 7) {
        $sql = mysqli_query($conn, "SELECT dock, dockIncrement FROM userfleet WHERE userID=$user[userID]"); // TODO: tady si vzít dock array, udělat for loop s $ammount, přidat increment value (primary key);
